...
#if !(hxluau)
#error "You need to install the 'hxLuau' library first!"
#end
...
class LuaScriptClass {
    ...
    public static macro function build(?scriptPath:String = null):Array<Field> {
        ...
        var lmCall:Field = {
            ...
            kind: FieldType.FFun({
                ...
                expr: macro {
                    ...
                    // select lua field
                    Lua.getglobal(__lua, func);

                    // catch errors (from psych engine)
                    var fieldType:Int = Lua.type(__lua, -1);
                    if (fieldType != Lua.TFUNCTION) {
                        if (fieldType > Lua.TNIL) {
                            var valueType = "unknown";
                            switch (fieldType) {
								case type if (type == Lua.TBOOLEAN): valueType = "boolean";
								case type if (type == Lua.TNUMBER): valueType = "number";
								case type if (type == Lua.TSTRING): valueType = "string";
								case type if (type == Lua.TTABLE): valueType = "table";
								case type if (type == Lua.TFUNCTION): valueType = "function";
								case type if (type == Lua.TINTEGER): valueType = "integer";
								case type if (type == Lua.TVECTOR): valueType = "vector";
								case type if (type == Lua.TBUFFER): valueType = "buffer";
                                default:
                                    if (fieldType <= Lua.TNIL)
                                        valueType = "nil";
                            }
                    ...
                    for (arg in args) {
                        psychlua.Convert.toLua(__lua, arg);
                    }

                    // try to call this function and return it's status
                    var status:Int = Lua.pcall(__lua, args.length, 1, 0);

                    // catch some errors (also from psych engine)
                    if (status != Lua.OK) {
                        var rawErr = Lua.tostring(__lua, -1);
                        var v = rawErr != null ? StringTools.trim(rawErr.toString()) : "";
                        if (v != "") {
                            switch (status) {
                                case type if (type == Lua.ERRRUN): Sys.println(__scriptPath + ": Runtime Error\n" + v);
                                case type if (type == Lua.ERRMEM): Sys.println(__scriptPath + ": Memory Allocation Error\n" + v);
                                case type if (type == Lua.ERRERR): Sys.println(__scriptPath + ": Critical Error\n" + v);
                    ...
                    var result:Dynamic = cast psychlua.Convert.fromLua(__lua, -1);
                    Lua.pop(__lua, 1);
                    return result;
                }
            }),
        ...
        var lmSet:Field = {
            ...
            kind: FieldType.FFun({
                ...
                expr: macro {
                    ...
                    psychlua.Convert.toLua(__lua, value);
                    Lua.setglobal(__lua, variable);
        ...
        var lmGet:Field = {
            ...
            kind: FieldType.FFun({
                ...
                expr: macro {
                    ...
                    Lua.getglobal(__lua, global);

                    var fieldType:Int = Lua.type(__lua, -1);
                    if (fieldType == Lua.TFUNCTION || fieldType <= Lua.TNIL) {
        ...
        var lmAddCallback:Field = {
            ...
            kind: FieldType.FFun({
                ...
                expr: macro {
                    ...
                    psychlua.Convert.addCallback(__lua, name, func);
                }
            }),
            pos: pos,
        };
            ...
            kind: FieldType.FFun({
                ...
                expr: macro {
                    ...
                    if (lumod.Lumod.storage.existsScript(__scriptPath)) {
                        ...
                        LuaL.dostring(lua, lumod.Lumod.storage.getScript(__scriptPath));
                    }
        ...
        var luaField:Field = {
            ...
            kind: FieldType.FVar(macro : State),
            pos: pos,
        };
